...
export default function PaginationCategories({ user }) {
    const { setPageNumberCategories, totalPages, pageNumberCategories } = UsePageNumber();
    const { categories } = useCategories();
    const pageCount = totalPages;

    const handlePageChange = (number) => {
        setPageNumberCategories(number.selected + 1);
    };
        ...
        <>
            {user && categories?.length > 0 && (
                <ReactPaginate
                    ...
                    nextLabel={
                        <span className={
                            `${(pageNumberCategories === totalPages && totalPages > 1) || totalPages === 1
                    ...
                    previousLabel={
                        <span className={
                            `${(pageNumberCategories === 1 && totalPages > 1) || totalPages === 1
                                ? 'opacity-50 cursor-not-allowed'
                                : 'opacity-[1]'
                            } 
                                h-full flex gap-[2px] items-center px-4 bg-blue text-white dark:bg-gray dark:text-white transition-all duration-300 ease-in-out text-[17px]`
                    ...
                    className="dark:bg-[#2b2828d4] bg-white shadowNav dark:text-white flex justify-center items-center gap-1 mt-4 relative"
                    nextClassName="next"
